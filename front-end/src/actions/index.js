...
export const createPost = (formValues, postArray) => async (dispatch, getState) => {
      ...
      const response = await axios.post("http://localhost:5001/api/putData", {
        id: idToBeAdded,
      ...
      await axios.post("http://localhost:5001/api/putComment", {
        id: idToBeAdded
      });
...
export const fetchComments = () => async dispatch => {
  fetch("http://localhost:5001/api/getComments")
      .then(data => data.json())
...
export const editPost = (idToUpdate, formValues, postArray) => async dispatch => {
    ...
    const response = await axios.post("http://localhost:5001/api/updateData", {
      id: objIdToUpdate,
      update: {
        message: formValues.message,
        name: formValues.name,
...
export const deletePost = (idTodelete, postArray) => async dispatch => {
    ...
    await axios.delete("http://localhost:5001/api/deleteData", {
      data: {
        id: objIdToDelete
      }
    });
...
export const deleteComment = (idTodelete, commentArray, title, userId) => async dispatch => {
  ...
  await axios.delete("http://localhost:5001/api/deleteComment", {
    data: {
      id: objIdToDelete,
      title: title,
      userId: userId
...
export const createComment = (userId, comment, postArray, googleId) => async dispatch => {
  ...
  const response = await axios.post("http://localhost:5001/api/addArrayElement",{
    id: objIdToDelete,
    comment: commentObj,
  });
